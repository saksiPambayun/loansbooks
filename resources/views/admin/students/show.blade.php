            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="max-w-xl">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="font-semibold dark:text-white">{{ $student->user->name }}</div>

                        <div class="text-gray-600 dark:text-gray-400">Email:</div>
                        <div class="font-semibold dark:text-white">{{ $student->user->email }}</div>

                        <div class="text-gray-600 dark:text-gray-400">NISN:</div>
                        <div class="font-semibold dark:text-white">{{ $student->nisn }}</div>

                        <div class="text-gray-600 dark:text-gray-400">Kelas:</div>
                        <div class="font-semibold dark:text-white">{{ $student->classroom->class_name ?? '-' }}</div>
                    </div>
                </div>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <span
                                            ...
                                            {{ $loan->latestStatus->status ?? 'Pending' }}
                                        </span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Belum ada riwayat
                                        peminjaman.</td>
                                </tr>
                            @endforelse
                        </tbody>
