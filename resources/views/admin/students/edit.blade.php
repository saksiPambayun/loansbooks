                    ...
                    <form action="{{ route('admin.students.update', $student) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Nama
                                    Lengkap</label>
                                <x-text-input id="name" name="name" type="text" class="block w-full" :value="old('name', $student->user->name)" required />
                                <x-input-error :messages="$errors->get('name')" class="mt-2" />
                            </div>
                            <div>
                                <label for="nisn" class="block text-sm font-medium text-gray-700 mb-2">NISN</label>
                                <x-text-input id="nisn" name="nisn" type="text" class="block w-full" :value="old('nisn', $student->nisn)" required />
                        ...
                        <div class="mb-8">
                            <label for="password" class="block text-sm font-medium text-gray-700 mb-2">Password
                                (Kosongkan jika tidak ingin ganti)</label>
                            <x-text-input id="password" name="password" type="password" class="block w-full" />
                            <x-input-error :messages="$errors->get('password')" class="mt-2" />

                            <label for="password_confirmation"
                                class="block text-sm font-medium text-gray-700 mb-2 mt-4">Konfirmasi Password
                                Baru</label>
